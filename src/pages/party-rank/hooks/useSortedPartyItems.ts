...
export const useSortedPartyItems = (
  ...
  return useMemo(() => {
    if (partyItems?.length > 0 && dbUsersRank?.length > 0) {
      ...
      const usersRank = dbUsersRank.filter((rank) => !hasMembers || members.includes(rank.userId));
      const rankBy: {
      ...
      usersRank.forEach((rank) => {
        ...
        if (rank.favoriteId) {
          ...
          rankBy.favoritesUsers[rank.favoriteId] = [...(rankBy.favoritesUsers[rank.favoriteId] || []), rank.userId];
        }
      ...
      return partyItems
        .map((item) => ({
          ...item,
          favoriteCount: rankBy.favorites[item._id] ?? 0,
          userLikesIds: rankBy.favoritesUsers[item._id],
          totalScore:
            (rankBy.grades[item._id] || []).reduce((acc, val) => acc + val, 0) +
            (rankBy.favorites[item._id] ?? 0) * 0.5,
          grade: rankBy.grades[item._id]
            ? rankBy.grades[item._id].reduce((acc, val) => acc + val, 0) / rankBy.grades[item._id].length
            : null,
        }))
        .sort((rankA, rankB) => {
